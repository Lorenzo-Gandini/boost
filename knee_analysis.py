...
def run_knee_analysis(athlete, show_plots):
    """
    Entry point for the knee analysis from main.
    """
    side = ask_joint_side("knee")

    if side in ["left", "both"]:
        analyze_single_knee(athlete, "left", show_plots)
    if side in ["right", "both"]:
        analyze_single_knee(athlete, "right", show_plots)


def analyze_single_knee(athlete, side, show_plots):
    """
    ...
    plot_distribution(
        ...
        os.path.join(output_folder, f"{athlete}_knee_{side}_peaks_distribution.png"),
        show_plots,
    )
    ...
    plot_distribution(
        ...
        os.path.join(output_folder, f"{athlete}_knee_{side}_valleys_distribution.png"),
        show_plots,
    )
    ...
    plot_polar_angles(
        ...
        "Setting 1 Peaks", "Setting 2 Peaks",
        os.path.join(output_folder, f"{athlete}_knee_{side}_polar_peaks.png"),
        show_plots,
    )
    ...
    plot_polar_angles(
        ...
        "Setting 1 Valleys", "Setting 2 Valleys",
        os.path.join(output_folder, f"{athlete}_knee_{side}_polar_valleys.png"),
        show_plots,
    )
    ...
    plot_angle_and_velocity_for_cycle(
        ...
        "Setting 1", "Setting 2",
        os.path.join(output_folder, f"{athlete}_knee_{side}_cycle_{cycle_index}_analysis.png"),
        show_plots,
    )

    # Save statistics
    user_message(f"All graphs for the {side} knee analysis have been saved in {output_folder}", "saving_graph")
    save_stats(stats1, stats2, athlete, "knee", side)
